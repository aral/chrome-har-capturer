...
class Page {
    ...
    _processEvents(client, fulfill, reject) {
        ...
        Network.requestWillBeSent((params) => {
            ...
            // Give ten second since the last request was sent to allow
            // additional content to load.
            /*
            this.timeLastRequestSent = new Date()
            setTimeout(()=>{this._checkFinished(fulfill)}, 10000)
            */
    ...
    _checkFinished(fulfill) {
        ...
        //const timeSinceLastRequestSent = new Date() - this.timeLastRequestSent

        //console.log(`Time since last request sent: ${timeSinceLastRequestSent}`)

        if (info.firstRequestMs &&
            info.domContentEventFiredMs &&
            info.loadEventFiredMs /* &&
            timeSinceLastRequestSent >= 9999*/ ) {
            fulfill();
        }
    }
}
